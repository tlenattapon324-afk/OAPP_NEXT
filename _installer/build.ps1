...
if ($Release) {
    Write-Step "Creating GitHub Release v1.3.0"
    $tag  = "v1.3.0"
    $notes = @"
## สิ่งที่เพิ่มใหม่ใน v1.3.0

### ฟีเจอร์ใหม่
- เพิ่มตัวเลือก "วันหยุดนักขัตฤกษ์" ตอนสร้างรายการจำกัดนัด — ระบบเช็คตาราง ``holiday`` อัตโนมัติแล้วรวมวันที่เข้ากับรายการที่จะสร้าง

### แก้ไขข้อบกพร่อง
- แก้บั๊กข้อความภาษาไทย (เช่นช่องหมายเหตุ) แสดงผลเพี้ยนหลังบันทึก โดยตั้งค่า charset ``tis620`` ของ connection ให้ตรงกับฐานข้อมูล HOSxP
...
"@
    gh release create $tag $exePath `
        --title "ระบบจำกัดนัดคลินิก v1.1.0" `
        --notes $notes
